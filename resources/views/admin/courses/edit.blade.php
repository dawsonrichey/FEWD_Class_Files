...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="handout">{{ trans('cruds.course.fields.handout') }}</label>
                <div class="needsclick dropzone {{ $errors->has('handout') ? 'is-invalid' : '' }}" id="handout-dropzone">
                </div>
                @if($errors->has('handout'))
                    <div class="invalid-feedback">
                        {{ $errors->first('handout') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.course.fields.handout_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="date_time">{{ trans('cruds.course.fields.date_time') }}</label>
                <input class="form-control datetime {{ $errors->has('date_time') ? 'is-invalid' : '' }}" type="text" name="date_time" id="date_time" value="{{ old('date_time', $course->date_time) }}" required>
...
<script>
    Dropzone.options.handoutDropzone = {
    url: '{{ route('admin.courses.storeMedia') }}',
    maxFilesize: 2, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').find('input[name="handout"]').remove()
      $('form').append('<input type="hidden" name="handout" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
    ...
    init: function () {
@if(isset($course) && $course->handout)
      var file = {!! json_encode($course->handout) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="handout" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
     error: function (file, response) {
